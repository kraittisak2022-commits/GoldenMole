/**
 * Public test hook for SMSOK (no Supabase user JWT).
 * Set verify_jwt = false in config.toml.
 *
 * Optional hardening: set Edge secret SMS_TEST_INVOKE_SECRET — then the client must send
 * header x-goldenmole-sms-test-secret with the same value (e.g. from mobile .env SMS_TEST_INVOKE_KEY).
 * If SMS_TEST_INVOKE_SECRET is unset, the endpoint is open to anyone with the project anon key.
 */
const corsHeaders: Record<string, string> = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-goldenmole-sms-test-secret",
};
...
Deno.serve(async (req) => {
  ...
  const testSecret = (Deno.env.get("SMS_TEST_INVOKE_SECRET") ?? "").trim();
  if (testSecret.length > 0) {
    const h =
      req.headers.get("x-goldenmole-sms-test-secret") ??
      req.headers.get("X-Goldenmole-Sms-Test-Secret") ??
      "";
    if (h !== testSecret) {

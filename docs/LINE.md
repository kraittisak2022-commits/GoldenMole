...
| เช็คชื่อ | ~~หลังบันทึก~~ → **09 / 13 / 17** (คนขับรถ + ท่าทราย) |
| รถดรัม / จำนวนเที่ยว | สรุปรถและเที่ยว (หลังบันทึก) |
| **การใช้รถ (09 / 13 / 17)** | สรุปดรัม + แม็คโคร — ข้ามถ้า 0 / ส่งอัปเดตเมื่อมีข้อมูลใหม่ (งบรวม ≤5/วัน) |
| **น้ำมันคงเหลือ (09 / 13 / 17)** | ถังหลัก + ถังสำรอง — ส่งอัปเดตเมื่อยอดเปลี่ยน (งบรวม ≤5/วัน) |
...
## สรุปการใช้รถ (ดรัม + แม็คโคร) — 09:00 / 13:00 / 17:00
...
- เวลา: ระบบ**ตรวจ**วันละ 3 รอบ (09:00 / 13:00 / 17:00 Asia/Bangkok) แต่**ส่งรายงานหลักวันละครั้ง** (เมื่อมีข้อมูลครั้งแรก)
- หลังจากนั้น ถ้ามีรายการใหม่ → ส่ง**เฉพาะรายการใหม่** (ไม่ส่งของเก่าซ้ำ)
- ถ้าไม่มีของใหม่ในรอบนั้น → **ไม่ส่ง** (ไม่สแปม)
- **งบวันละ ≤5 ข้อความ** รวมทุก digest (รถ / น้ำมัน / เช็คชื่อ) — ครบแล้วข้ามจนกว่าวันถัดไป (`force` ข้ามงบได้)
- ไม่แสดงบรรทัดที่จำนวน 0 (เช่น ไม่ขึ้น “แม็คโคร 0 คัน”)
...
## น้ำมันคงเหลือ — 09:05 / 13:05 / 17:05 (ถังหลัก + ถังสำรอง)
...
- เวลา: **09:05 / 13:05 / 17:05 Asia/Bangkok** (offset จากรายงานรถ)
- ยอดไม่เปลี่ยน → ไม่ส่งซ้ำ; ยอดเปลี่ยนหลังเติม/เบิก → ส่ง `อัปเดต`
- ร่วมงบวันละ ≤5 ข้อความกับ digest อื่น
...
## เช็คชื่อ — 09:10 / 13:10 / 17:10 (คนขับรถ + พนักงานท่าทราย)

Edge **`notify-daily-attendance`** อ่านบันทึก Labor/Leave ของวันนั้นแล้วส่งรายงานรวม

- **ไม่แจ้งหลังบันทึกในแอปแล้ว** — เหลือเฉพาะรอบอัตโนมัติ 3 ครั้ง/วัน
- เวลา: **09:10 / 13:10 / 17:10 Asia/Bangkok**
- ยังไม่มีข้อมูล → ไม่ส่ง; มีรายชื่อใหม่/เปลี่ยน → ส่งอัปเดต
- ร่วมงบวันละ ≤5 ข้อความกับ digest อื่น
- ทดสอบไม่เข้ากลุ่ม: `{ "force": true, "testPersonalOnly": true }`
...
### AI (OpenRouter) + ฐานข้อมูลเว็บแอป
...
- AI เป็น**ที่ปรึกษาคุยสองทาง** (โทนกันเอง ถามกลับได้) และจำบทสนทนาสั้นๆ ต่อคน
- พิมพ์ `เริ่มใหม่` เพื่อล้างประวัติแชท
- AI **เรียกเครื่องมืออ่าน DB** (ชุดเดียวกับเว็บ) ตามคำถาม: น้ำมัน, เช็คชื่อ/ลา, เบิกเงิน, รถ, บำรุงรักษา, แผนงาน, ท่าทราย, พนักงาน
- ตอบคำตอบเดียวตรงๆ (ถ้า reply token หมดอายุเพราะ AI ช้า จะส่งด้วย push แทน)
- อ่านอย่างเดียว — ไม่แก้ข้อมูลผ่าน LINE

คุยภาษาธรรมชาติได้ เช่น:

- สวัสดี / วันนี้หน้างานเป็นยังไง
- ใครเบิกเงินสัปดาห์นี้ ช่วยดูให้หน่อย
- น้ำมันเหลือเท่าไหร่ มีการเบิกอะไรบ้าง
- มีแจ้งซ่อมอะไรช่วงนี้ ควรจัดคิวยังไง
- แผนงานวันนี้ทำอะไร

คำลัด (fallback): `ช่วย` / `เริ่มใหม่` / หรือคำเดิมอย่าง `น้ำมัน` `เช็คชื่อ` `รถ` `สรุป`
